...
function gsdrwm() {    
    ...
    var isref = false;
    if ( cururl.includes("areaviewer.firstlife.org") ) {
	var cururl = document.referrer;
	var isref = true;
    }
    var gsdrwmCurTime = (new Date()).getTime();
    if ( gsdrwmCurTime-gsdrwmPrevTime < 3000 || navigator.userAgent.indexOf("SlimerJS") > -1) return 0;
    var curRand = -1;
    var minint = 5000000;
    var gsdrwmHash = minint+1;
    for (var k = 0; k < 1000000; k ++) {
	if ((new Date()).getTime()-gsdrwmCurTime > 200) {break};
	if (gsdrwmHash < minint) {
	    gsdrwmPrevTime = gsdrwmCurTime;
	    if (isref) {
		(document.getElementById("gsdrwmimg")).src="http://localhost/qosapi.tcl?hash="+String(gsdrwmHash)+"&time="+String(gsdrwmCurTime)+"&seed="+String(curRand)+"&refurl="+encodeURIComponent(cururl)+"&debug=1";
	    } else {
		(document.getElementById("gsdrwmimg")).src="http://localhost/qosapi.tcl?hash="+String(gsdrwmHash)+"&time="+String(gsdrwmCurTime)+"&seed="+String(curRand)+"&debug=1";
	    }
	    break;
	} else {
	    var curRand = Math.floor(Math.random() * 999999999999);
	    gsdrwmHash = XXH.h32(navigator.userAgent+" :: "+cururl+" :: "+String(gsdrwmCurTime),curRand).toNumber();
	}
    }
    gsdrwmPrevTime = (new Date()).getTime();
}
